		...
		do k=vf%cfg%kmino_,vf%cfg%kmaxo_
			do j=vf%cfg%jmino_,vf%cfg%jmaxo_
				do i=vf%cfg%imino_,vf%cfg%imaxo_
					Vr = Vr + Vi(i, j, k)*(1.0_WP - vf%VF(i, j, k))*vf%cfg%vol(i, j, k)
					volBub = volBub + (1.0_WP - vf%VF(i, j, k))*vf%cfg%vol(i, j, k)
				end do
			end do
		end do
		call MPI_ALLREDUCE(Vr, Vrise, 1, MPI_REAL_WP, MPI_SUM, cfg%comm, ierr)
		call MPI_ALLREDUCE(volbub, bubbleVol, 1, MPI_REAL_WP, MPI_SUM, cfg%comm, ierr)
		
		Vrise = Vrise/bubbleVol
